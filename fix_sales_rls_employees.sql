-- السماح للموظفين الذين لديهم صلاحية "sales" برؤية وإدخال مبيعات عمارات المالك
-- يجب تنفيذ هذا السكربت في Supabase → SQL Editor حتى يظهر سجل المبيعات للموظفين ذوي الصلاحية
...
CREATE POLICY "sales_select_policy" ON sales FOR SELECT USING (
  ...
  OR EXISTS(
    ...
    WHERE b.id = sales.building_id AND COALESCE((e.permissions->>'sales')::boolean, false) = true
  )
);
